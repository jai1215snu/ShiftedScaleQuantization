...
def QuantRecursiveShiftRecon(model: nn.Module, layerEnabled, qnn, test_loader, prv_name="", **kwargs):
    for name, module in model.named_children():
        ...
        if isinstance(module, QuantModule):
            ...
            if curName in layerEnabled:
                run_ShiftRecon(model, curName, module, qnn, test_loader, **kwargs)
            ...
            if curName in layerEnabled:
                ...
                QuantRecursiveShiftRecon(module, layerEnabled, qnn, test_loader, curName, **kwargs)
        else:
            QuantRecursiveShiftRecon(module, layerEnabled, qnn, test_loader, curName, **kwargs)
...
def run_ShiftRecon(model, curName, module, qnn, test_loader, **kwargs):
    if isinstance(module, QuantModule):
        layer_recon_shiftedScale(module, kwargs['iters'], kwargs['lmda'], qnn, test_loader)
    elif isinstance(module, QuantBasicBlock):
        block_recon_shiftedScale(module, kwargs['iters'], kwargs['lmda'], qnn, test_loader)
    else:
...
if __name__ == '__main__':
    ...
    kwargs['iters'] = itr
    kwargs['lmda'] = (10)**(-lmda)
    channelShift_wLoss(test_loader, train_loader, cali_data, botInfo, kwargs, args)
