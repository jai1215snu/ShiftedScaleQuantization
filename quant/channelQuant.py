...
class ChannelQuant(nn.Module):
    def __init__(self, uaq: UniformAffineQuantizer, weight_tensor: torch.Tensor, shuffle_ratio: float = 0.0, qscale: float = 0.5):
        ...
        self.qscale = qscale
        self.nchannel = weight_tensor.shape[0]
    ...
    def init_resol(self, prob: float = 0.1):
        self.sel_resol = torch.multinomial(torch.tensor([1-prob, prob]), num_samples=self.nchannel, replacement=True)
    ...
    def changeResol(self, pos: int):
        self.resetResol()
        self.setResol(pos)
    
    def setResol(self, pos: int, value: float = 1.0):
        self.simpleScale[pos] = value
